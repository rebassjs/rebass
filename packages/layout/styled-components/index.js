...
var widthToColumns = function widthToColumns(width) {
  return Array.isArray(width) ? width.map(widthToColumns) : "repeat(auto-fit, minmax(".concat(px(width), ", 1fr))");
};

var countToColumns = function countToColumns(n) {
  return Array.isArray(n) ? n.map(countToColumns) : "repeat(".concat(n, ", 1fr)");
};
