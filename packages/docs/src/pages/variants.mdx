...
Most components will use style objects defined in `theme.variants`,
but the `Text` and `Heading` components use the `theme.text` object, and `Button` uses `theme.buttons`.
