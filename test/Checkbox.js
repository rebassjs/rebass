
import React from 'react'
import test from 'ava'
import { shallow } from 'enzyme'
import { Checkbox, Label, theme } from '../src'

let wrapper
let inner

const { fontSizes } = theme

test('renders', t => {
  wrapper = shallow(<Checkbox />)
  inner = wrapper.first().shallow()
})

test('is a div', t => {
  t.true(inner.is(Label))
})

test('has a className', t => {
  t.is(inner.props().className, 'Checkbox')
})

test('accepts custom className props', t => {
  wrapper = shallow(<Checkbox className='foo' />)
  inner = wrapper.first().shallow()
  t.is(inner.props().className, 'Checkbox foo')
})

test('accepts custom styles', t => {
  wrapper = shallow(<Checkbox style={{ color: 'tomato' }} />)
  inner = wrapper.first().shallow()
  t.is(inner.props().style.color, 'tomato')
})

test('context styles override default styles', t => {
  wrapper = shallow(<Checkbox />, {
    context: {
      rebass: {
        Checkbox: {
          marginLeft: 0
        }
      }
    }
  })
  inner = wrapper.first().shallow()
  t.is(inner.props().style.marginLeft, 0)
})

test('style props override context styles', t => {
  wrapper = shallow(
    <Checkbox
      color='blue'
      style={{
      color: 'tomato'
    }} />, {
    context: {
      rebass: {
        Arros: {
          color: 'magenta'
        }
      }
    }
  })
  inner = wrapper.first().shallow()
  t.is(inner.props().style.color, 'tomato')
})

